...
class APIs {
  ...
  static Future<ChatUser?> getCompany() async {
    try {
...
static Future<void> getFirebaseMessagingToken() async {
  ...
  await fMessagin.getToken().then((token) {
    if (token != null) {
      me.pushToken = token;  // Ensure to store the token in the database
      log('PushToken: $token');
      // Update the token in Firestore for the user
      firesotre.collection('users').doc(user.uid).update({'push_token': token});
    }
  });
